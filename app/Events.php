...
<div class="container" style="margin-top: 25px;">
  ...
  ?>
    ...
    if (isset($_POST['btn_search']) && !empty($_POST['search'])) {
      $query = "SELECT * FROM event WHERE event_name LIKE '%$_POST['search']%'";
      $result = mysqli_query($conn, $query);
      if(mysqli_num_rows($result) > 0)
      {

        echo '<h3>Showing results for:<font color="red"> $_POST['search']</font></h3>';
    ...
    <?php
        ...
        echo '<h3 style="color: red; text-align: center;">No Recorded Found!</h3>';
      }
    ?>
  </div>
</div>
